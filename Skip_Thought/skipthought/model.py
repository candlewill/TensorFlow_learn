...
class SkipthoughtModel:
    ...
    def _create_placeholders(self):
        with tf.variable_scope('placeholders'):
            ...
            self.prev_decoder_weights = [
                tf.placeholder(tf.float32, shape=[None, ], name="prev__decoder_weight{0}".format(i))
                for i in range(self.max_length_decoder)]
            ...
            self.next_decoder_weights = [
                tf.placeholder(tf.float32, shape=[None, ], name="next__decoder_weight{0}".format(i))
                for i in range(self.max_length_decoder)]
    ...
    def _create_decoder(self, scope_name, encoder_state, decoder_input):
        ...
        loop_function_predict = tf.nn.seq2seq._extract_argmax_and_embed(self.embedding_matrix, output_projection=(w, b),
