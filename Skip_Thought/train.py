...
def main(**kwargs):
    ...
    if kwargs['init_from'] is not None:
        ...
        assert os.path.isfile(
            os.path.join(kwargs['init_from'], "config.pkl")), "config.pkl file does not exist in path %s" % kwargs[
            'init_from']
        assert os.path.isfile(
            os.path.join(kwargs['init_from'], "textdata.pkl")), "textdata.pkl file does not exist in path %s" % kwargs[
            'init_from']
        ...
        with open(os.path.join(kwargs['init_from'], 'config.pkl'), 'rb') as f:
            ...
            for checkme in need_be_same:
                assert saved_model_args[checkme] == kwargs[
